...
# get php version
read -e -p $'Enter \e[33mPHP-Version\033[0m (7.4, 8.0, 8.1, 8.2 , 8.3):\n' -i "8.3" STACK_PHP_VERSION
...
# ############################################## end getting vars
rm -rf .git .dev
mkdir -p ./data/{mongo,mysql,mariadb,pgsql,redis}

# create .env file
cp .env.example .env

# create src folder and copy .env.example to src/.env
git clone --depth=1 --branch=main https://github.com/ariadata/laravel-11-for-larastack.git src
rm -rf ./src/.git
...
read -e -p $'Choose one of \e[33mmysql,mariadb,pgsql\033[0m for your RDBMS? \n' -i "mariadb" DC_RDBMS
while [ "$DC_RDBMS" != "mysql" ] && [ "$DC_RDBMS" != "pgsql" && [ "$DC_RDBMS" != "mariadb" ]; do
    read -e -p $'Choose one of \e[33mmysql,mariadb,pgsql\033[0m for your RDBMS? \n' -i "mysql" DC_RDBMS
done
...
if [ "$DC_USE_MONGO" != "y" ]; then
    # clear all lines between "##-- MONGO BEGIN --##" and "##-- MONGO END --##" in docker-compose.yml file
    sed -i '/^##-- MONGO BEGIN --##$/,/##-- MONGO END --##/d' docker-compose.yml
